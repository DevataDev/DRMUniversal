...
function WriteSegment($ChID, $ChName, $Keys, $aHeader, $aData, $vHeader, $vData, $DownloadIndex, $aFifos, $vFifo)
{
    global $WorkPath, $BinPath;
    global $FFMpegCMD;
    global $DeleteEncryptedAfterDecrypt;
    ...
    $cmd = $FFMpegBin . " -hide_banner -y -fflags +igndts -copyts -i $VideoDecFileName $strAudioIn -map 0:v $map -c:v copy -c:a copy -f mpegts $namedPipe";
    DoLog("Merging Command : $cmd");
    DoLog("Merging segment .... please wait .....");
    $Res = null;
    exec($cmd, $Res);
    // DoLog("Output : " . implode("", $Res));
    // foreach ($Res as $r) {
    //     fwrite($Merged_Fifo, implode("\n", $Res));
    // }
    // stream_set_blocking($Merged_Fifo, 0);
    fwrite($Merged_Fifo, implode("", $Res));
...
function StartFFMPEG($ChName, $ChID, $audioCount = 1)
{
    global $WorkPath, $BinPath;
    global $PlaylistLimit;
    global $FFMpegPID;
